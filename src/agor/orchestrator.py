"""
Orchestration service for AGOR bundle creation.
...
class Orchestrator:
    """Main orchestration service for AGOR bundle creation."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize orchestrator with repository and options.
    ...
    async def bundle(self, output_path: Optional[Path] = None) -> Path:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                log.error("Bundle creation failed", error=str(e), repo_url=self.repo_url)
    ...
    async def _add_agor_tools(self, bundle_dir: Path) -> None:
        ...
        tools_dir = bundle_dir / "tools_for_ai"
        tools_dir.mkdir()
        
        # Copy AGOR tools
        agor_tools_src = Path(__file__).parent / "tools"
        if agor_tools_src.exists():
            shutil.copytree(agor_tools_src, tools_dir / "agor_tools")
...
async def create_bundle(
    ...
    **kwargs
    ...
    Returns:
        Path to the created bundle file
    """
    orchestrator = Orchestrator(repo_url, **kwargs)
    return await orchestrator.bundle(output_path)
