...
class AgorSettings(BaseSettings):
    ...
    default_shallow_depth: int = Field(
        default=5,
        description="Default depth for shallow git clones"
    )
    ...
    compression_format: str = Field(
        default="zip",
        description="Default compression format for bundles"
    )
    preserve_history: bool = Field(
        default=False,
        description="Whether to preserve full git history in bundles"
    )
    ...
    interactive: bool = Field(
        default=True,
        description="Whether to show interactive prompts"
    )
    assume_yes: bool = Field(
        default=False,
        description="Whether to assume yes for all prompts"
    )
    clipboard_copy_default: bool = Field(
        default=True,
        description="Whether to copy prompts to clipboard by default"
    )
