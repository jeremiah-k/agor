...
from .strategy_protocols import get_strategy_protocol
...
class AgentCoordinationHelper:
    ...
    def _parse_parallel_divergent_strategy(self, content: str) -> Dict:
        ...
        agent_assignments = []
        agent_pattern = r"### (Agent\d+) Assignment.*?Branch.*?`([^`]+)`.*?Status.*?([^\n]+)"
    ...
    def _parse_pipeline_strategy(self, content: str) -> Dict:
        ...
        current_stage_match = re.search(r"### Status: Stage (\d+) - ([^(]+) \(ACTIVE\)", content)
    ...
    def _determine_agent_role(self, strategy_info: Dict, agent_id: Optional[str]) -> Dict:
    ...
    def _determine_pipeline_role(self, strategy_info: Dict, agent_id: Optional[str]) -> Dict:
    ...
    def _determine_swarm_role(self, strategy_info: Dict, agent_id: Optional[str]) -> Dict:
...
def check_strategy_status() -> str:
    ...
    if agentconvo_file.exists():
        ...
        recent_lines = [line for line in lines[-5:] if line.strip() and not line.startswith('#')]
        if recent_lines:
