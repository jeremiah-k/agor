...
2. **Create Internal Checklist**: Automatically create and maintain an internal checklist for session tracking
3. **Select Appropriate Role**: Choose the correct AGOR role based on task requirements
4. **Follow AGOR Protocols**: Use structured workflows, hotkeys, and coordination methods
5. **Create Snapshots**: Always create snapshots before ending sessions using proper AGOR format
6. **Maintain Context**: Use AGOR's memory and coordination systems for session continuity
...
### Step 2: Create Internal Checklist
```python
from agor.tools.enhanced_hotkeys import start_session
from agor.tools.agent_checklist_system import create_checklist

# Automatically detect session start and create internal checklist
start_session(user_input, agent_role)
# This creates an internal checklist with mandatory AGOR items
# plus auto-generated custom items based on task analysis
```
...
```python
from agor.tools.enhanced_hotkeys import checklist_status, process_hotkey

# Check your internal checklist progress
status = checklist_status()
print(status)

# Hotkeys automatically update checklist
result = process_hotkey("commit", "Fixed authentication bug")
# This automatically marks "frequent_commits" as complete
```

### Snapshot Requirements (MANDATORY)
...
```python
from agor.tools.enhanced_hotkeys import end_session, create_snapshot

# System automatically enforces snapshot creation
if end_session(user_input):
    # Session can end safely - snapshot was created
    pass
else:
    # Must create snapshot first
    create_snapshot("Additional context about work completed")
```
...
- **Checklists automatically saved to memory branches**
...
1. **NEVER end a session without creating a snapshot** - This is automatically enforced
...
8. **Trust the internal checklist system** - It tracks your progress automatically

## 🎯 Success Criteria

You are successfully using AGOR when you:
- ✅ Read AGOR documentation before starting work
- ✅ Create internal checklist automatically at session start
...
**Remember**: AGOR transforms AI assistants into sophisticated development coordinators. Your adherence to these protocols ensures effective coordination, context preservation, and successful project outcomes. The internal checklist system automatically tracks your progress and ensures you follow all mandatory procedures.
