...
**For AI agents with direct git access and file system capabilities (e.g., Jules by Google, Augment Code Remote Agents).**

These agents operate directly on a cloned repository and do not use pre-packaged bundles.

**Initialization Protocol for Direct Access Agents (like Jules):**
...
> **📖 For detailed setup and operational guidance for Jules-like agents, please refer to our [Jules Direct Access Guide](docs/jules-direct-access-guide.md).**
...
agor bundle repo --sqlite                   # With SQLite memory. The --sqlite bundle option ensures the SQLite binary and necessary support are included in the bundle, as it's a component of the backend memory synchronization system. It does not enable direct agent hotkeys for SQLite memory operations, which are managed via markdown files and snapshots.
