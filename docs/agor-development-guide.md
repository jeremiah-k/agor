...
**✅ CLARIFICATION**: Snapshot documents are stored in `.agor/agents/{agent_id}/snapshots/` on memory branches.
...
- **Generated by**: Agents using dev tools functions like `create_development_snapshot()`.
- **Used by**: Agents receiving work or by the same agent resuming work.
- **Format**: Structured markdown with comprehensive technical and project context.
- **Implementation**: `snapshots.py` and `snapshot_templates.py` (generates these snapshot documents).
- **Memory Management**: All snapshot operations handled automatically by dev tools on memory branches.
